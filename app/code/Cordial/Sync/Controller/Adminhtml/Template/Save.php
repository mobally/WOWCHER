...
class Save extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            $model = $this->_objectManager->create(\Cordial\Sync\Model\Template::class)->load($id);
            if (!$model->getId() && $id) {
            ...
            try {
                $model->save();
                $this->messageManager->addSuccessMessage(__('You saved the Map.'));
                $this->dataPersistor->clear('cordial_sync_template');
                ...
                $this->messageManager->addExceptionMessage($e, __('Something went wrong while saving the Map.'));
            }
        
            $this->dataPersistor->set('cordial_sync_template', $data);
